...
module.exports = {
  ...
  add: async (values) => {
    ...
    const entry = await Homepage.create(data);

    // Create relational data and return the entry.
    return Homepage.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  edit: async (params, values) => {
    // Extract values related to relational data.
    const relations = _.pick(values, Homepage.associations.map(a => a.alias));
    const data = _.omit(values, Homepage.associations.map(a => a.alias));

    // Update entry with no-relational data.
    const entry = await Homepage.updateOne(params, data, { multi: true });
  ...
  remove: async params => {
    ...
    await Promise.all(
      Homepage.associations.map(async association => {
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Homepage.attributes).reduce((acc, curr) => {
      switch (Homepage.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
